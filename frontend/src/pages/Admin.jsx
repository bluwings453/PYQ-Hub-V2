          ...
          <div key={p._id} className="border border-hairline rounded-md p-4 bg-surface">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="flex gap-2 shrink-0">
                <button
                  onClick={() => setPreviewId(previewId === p._id ? null : p._id)}
            ...
            {previewId === p._id && (
              <div className="mt-4 space-y-2">
                {p.fileType === "pdf" ? (
                  <iframe title={`preview-${p._id}`} src={p.files[0]?.url} className="w-full h-96 border border-hairline rounded-sm" />
                ) : (
                  p.files.map((f, i) => (
                    <img key={i} src={f.url} alt={`Page ${i + 1}`} className="w-full border border-hairline rounded-sm" />
                  ))
                )}
              </div>
            )}
