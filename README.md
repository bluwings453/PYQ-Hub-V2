...
**Prerequisites:** Node.js 18+, your MongoDB Atlas connection string.
...
- **A paper is either one PDF, or one-or-more photos** of the pages, in the order they
  ...
  its own.
- **Files are stored on the server's own disk**, under `backend/uploads/`, via Multer.
  This is the simplest possible setup and is fine for local development and for getting
  started. **Known limitation:** on Render's free tier (and most free hosting), this
  folder does not persist across redeploys or restarts — uploaded files can be wiped
  without warning. Fine to accept for now; revisit with Cloudinary or similar once
  things are calmer.
...
- **Every new paper is `pending` until a moderator approves it** at `/admin`. The admin
  queue previews every page (fetched through an authenticated endpoint, since pending
  files aren't public) before you decide.
